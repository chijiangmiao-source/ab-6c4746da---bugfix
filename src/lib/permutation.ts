...
/**
 * 相邻断点数：在首尾哨兵 0、n+1 之间，符号标记不再连续递增的位置数。
 * 它可作为倒位搜索时衡量局部排列连续性的轻量级特征。
 */
export function adjacencyBreakpointCount(tokens: readonly Token[]): number {
  let previous = 0;
  let count = 0;
  for (const token of tokens) {
    const value = tokenToSigned(token);
    if (value - previous !== 1) count += 1;
    previous = value;
  }
  if (tokens.length + 1 - previous !== 1) count += 1;
  return count;
}
